...
│   ├── guard_staff_*.sql (6) + rollback_guard_staff_*.sql (6)  Guard 0 (service_role por auth.role() | super_admin | fn_is_staff) + guard de club en las 6 RPC sensibles; cierra el vector del portal del 22/09. APLICADAS: liberar_linea, desoficializar_carrera, aplicar_resultado. PENDIENTES (camino de pago, esperan OK): fn_siguiente_recibo, anular_recibo, emitir_recibo
