...
│   └── rpc_cambiar_monta.sql       ISSUE-084 — RPC rpc_cambiar_monta + trigger trg_insc_monta_oficial (monta en carrera oficial: guard de plata comprometida + borra lo pagable del saliente + recalcular); rollback_rpc_cambiar_monta.sql; NO APLICADA en prod (rama fix/issue-084-montas-post-oficial, 22/09)
...
node tests/probe_montas_post_oficial.mjs            # ISSUE-084 — rpc_cambiar_monta + trigger + saveMontas real: A1–A11 (impago/retenido/pagado con y sin recibo/incentivo/provisional/recálculo fallido/update directo/payload entero/set_config), 7/7 mutantes (--mutantes; los de SQL con PSQL_CMD en el sandbox tests/local/); ESCRIBE en la 9999 y la restaura entera (ids incluidos); necesita la migración aplicada (corta antes de tocar nada si no está)
