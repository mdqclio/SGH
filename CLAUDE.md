...
│   ├── guard_staff_*.sql (6) + rollback_guard_staff_*.sql (6)  Guard 0 (service_role por auth.role() | super_admin | fn_is_staff) + guard de club en las 6 RPC sensibles; cierra el vector del portal del 22/09. LAS SEIS APLICADAS: liberar_linea/desoficializar_carrera/aplicar_resultado el 22/09; fn_siguiente_recibo/emitir_recibo/anular_recibo el 23/09 (md5 de pg_get_functiondef verificado en cada una; probe 60/60, 8/8 mutantes, smoke con sesión staff real en la 9999). ISSUE-090 cerrado
│   ├── reunion_liquidacion_cerrada.sql  ISSUE-091 — reuniones.liquidacion_cerrada_at + triggers (líneas/headers de reunión cerrada: sólo service_role o migración; cerrar/reabrir: sólo super_admin) + cierre de R6/R8 TAL COMO ESTÁN en la misma transacción; rollback_reunion_liquidacion_cerrada.sql; **APLICADA 2026-09-25** (`20260925163044`; md5 de las 3 funciones verificado; INSERT en R6 por la API → P0091)
│   ├── revoke_fn_reunion_liq_cerrada.sql  SEGURIDAD — REVOKE EXECUTE de fn_reunion_liq_cerrada a PUBLIC y anon (authenticated/service_role lo conservan); rollback_revoke_fn_reunion_liq_cerrada.sql; APLICADA 2026-09-25 (`20260925165014`)
│   ├── sanciones_insert_update_staff.sql  SEGURIDAD — sanciones_insert/_update exigen fn_is_staff() además del club (un usuario de portal con club podía insertar y editar/revocar sanciones sobre sí mismo); md5 de las políticas en el encabezado; rollback_sanciones_insert_update_staff.sql; **NO APLICADA** (PR fix/sanciones-alta-club-staff)
...
node tests/probe_pagos_rol_carrera.mjs    # rol y nº de carrera en el tab Pagos (65 asserts); 1a/1c por comportamiento con fixture multi-rol en la 9999 (ESCRIBE: guard que se niega fuera de la 9999, restore por estado + conteos; club_secuencias se devuelve SÓLO por compare-and-set si nadie emitió en el medio — si no, aviso, no rojo; limpia en finally/SIGINT y barre restos de un kill -9 al arrancar; --abortar=<paso> / --pausar=<seg> para probarlo); --mutantes 11/11; sin baselines fijos (GOTCHA #100)
...
node tests/probe_guard_staff_rpcs.mjs [--fn <nombre>]   # guard de staff en las 6 RPC + rpc_cambiar_monta: matriz función × 7 perfiles (service_role, super_admin, secretario, operador, operador de otro club, portal, authenticated sin fila en usuarios, anon) con argumentos VÁLIDOS de la 9999; el 42501 tiene que venir del guard de ESA función (observable por capa, GOTCHA #86); --fn corre una sola (para después de cada apply_migration); --mutantes 8/8 con PSQL_CMD en el sandbox tests/local/; ESCRIBE en la 9999 y restaura (ojo: borrar auditoría y recibos ANTES que los usuarios, hay FK)
node tests/smoke_pago_staff_9999.mjs                # SMOKE del camino de pago con SESIÓN STAFF REAL (magiclink, no service_role): emitir_recibo + verificación + anular_recibo sobre la 9999; 15/15; ESCRIBE (recibo, líneas, usuario, club_secuencias) y restaura TODO por estado, secuencia incluida
