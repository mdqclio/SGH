...
node tests/probe_caballeriza_provisorio.mjs        # caballerizas.html: titular opcional (todo-o-nada), edición no reescribe responsables si no cambiaron, alta sin titular → rpc_caballeriza_provisorio; RPC con sesiones reales (staff/portal/otro club), idempotencia, homónimos, re-derivación; --mutantes: 6 HTML automáticos + 9 SQL por gemela (execute_sql); --solo-ui; CABALLERIZAS_HTML acepta URL
node tests/probe_modificar_inscripcion_portal.mjs   # Modificar desde el portal: rpc_modificar_inscripcion (guards = baja, cadena del propietario, GATE-1=B, ratificado exige jockey) + UI (botón con predicado de Retirar, confirm de entrenador / caballeriza sin titular, toast sin_propietario); fixture 9987/9986 + teardown por estado; --mutantes: 9 de portal automáticos + 15 de SQL por MCP sobre gemela; PORTAL_HTML acepta URL (HTML servido)
node tests/probe_carta_numero_turno.mjs [out_dir]  # carta-llamados PDF: título `TURNO N — condición` con numero_turno (no numero_carrera_programa; R9 T3=7 discrimina) + ancho con Chromium sobre el CSS de impresión real: nadie desborda, T5–T8 a 2 líneas, el chip de distancia no se empuja; PNG del bloque; sólo lectura; CARTA_HTML acepta URL; necesita ~/chromium-libs + fonts-liberation (docs/SERVER.md)
node tests/probe_mandil_colores.mjs     # partidor-colors.js contra el nomenclador oficial de Fede (18/09): fondo por HSL, número por clase, estructura 1..16 + fallback gris, consumidores; mutante = paleta 234a833; sin Supabase ni browser; PARTIDOR_JS acepta URL
node tests/probe_fmtinput_onblur.mjs    # liquidaciones.html: los 4 inputs de monto van por onblur (no oninput) — tipeo dígito por dígito no pierde ceros, caso Valeria, payload de saveReparto con sb stub; mutante = main pre-fix; sin Supabase ni browser; LIQUIDACIONES_HTML acepta URL
...
**Fuente del sistema:** lo que el CSS pide como `Arial` lo resuelve fontconfig. Sin `fonts-liberation`
cae en DejaVu Sans (~10% más ancha) y las celdas que "envuelven" son más que en la impresora real;
desde el 17/09 está instalada en `~/.local/share/fonts` (ver docs/SERVER.md). `probe_carta_numero_turno.mjs`
tiene un canario que falla si `Arial` no mide como Liberation.
