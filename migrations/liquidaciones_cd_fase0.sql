-- ============================================================================
-- ISSUE-001 — Liquidaciones C+D · Fase 0 (migración de schema)
-- Branch: feat/liquidaciones-cd
-- Arquitectura C1: la LÍNEA (liquidacion_detalle) es la unidad de deuda con su
-- estado; `recibos` (nueva) es la capa de cobro que agrupa líneas impagas
-- cross-reunión vía recibo_id. Beneficiario denormalizado en la línea.
--
-- Correr en el SQL Editor de Supabase (el MCP es read-only). NO ejecutar hasta
-- que Leo valide. Idempotente (re-ejecutable).
-- ============================================================================
...
-- ─────────────────────────────────────────────────────────────────────────
-- 2. Config de liquidación por club (saca lo hardcodeado del JS)
--    % reparto 1°-5° + 2% fondo solidario = 100. Incentivos por reunión.
--    Días anti-doping. Retención DGI nullable (Dolores no retiene; sin escala).
-- ─────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS liquidacion_config (
  ...
  retencion_dgi_pct        NUMERIC(6,3) NULL,   -- nullable: Dolores no retiene. Escala/cálculo: fuera de alcance ahora.
...
-- Seed para Dolores con los % confirmados (incentivos en 0 hasta que Fede dé el monto)
...
-- ─────────────────────────────────────────────────────────────────────────
-- 3. Numeración correlativa de recibo POR CLUB (secuencia + función con lock)
-- ─────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS club_secuencias (
  club_id        UUID NOT NULL REFERENCES clubs(id),
  tipo           TEXT NOT NULL,            -- 'recibo'
...
BEGIN
  -- INSERT … ON CONFLICT DO UPDATE bloquea la fila → incremento atómico, sin race.
...
-- ─────────────────────────────────────────────────────────────────────────
-- 4. Recibos (capa de cobro, cross-reunión, por persona)
-- ─────────────────────────────────────────────────────────────────────────
CREATE TABLE IF NOT EXISTS recibos (
  id                 UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  club_id            UUID NOT NULL REFERENCES clubs(id),
  numero_recibo      INTEGER NOT NULL,                  -- correlativo por club (fn_siguiente_recibo)
  beneficiario_tipo  beneficiario_tipo NOT NULL,
  profesional_id     UUID NULL REFERENCES profesionales(id),
  propietario_id     UUID NULL REFERENCES propietarios(id),
  ...
  retencion_dgi      NUMERIC(15,2) NULL,                -- nullable: Dolores no retiene
  neto_a_cobrar      NUMERIC(15,2) GENERATED ALWAYS AS
                       (total_premios - total_descuentos - COALESCE(retencion_dgi, 0)) STORED,
  cobrador_nombre    TEXT NULL,
  cobrador_documento TEXT NULL,                         -- DNI del que cobra
  comprobante_url    TEXT NULL,                         -- transferencia (escaneo)
  estado             estado_recibo NOT NULL DEFAULT 'emitido',
  emitido_por        UUID NULL REFERENCES usuarios(id),
 ...
 OR (beneficiario_tipo = 'propietario' AND propietario_id IS NOT NULL AND profesional_id IS NULL)
  )  -- 'club' no emite recibo (fondo solidario)
);

-- ─────────────────────────────────────────────────────────────────────────
-- 5. La LÍNEA como unidad de deuda — ampliar liquidacion_detalle
--    (carrera_id ya existe y es nullable → sirve para incentivos por-reunión)
-- ─────────────────────────────────────────────────────────────────────────
...
ALTER TABLE liquidacion_detalle ADD COLUMN IF NOT EXISTS posicion          INTEGER;          -- para regla anti-doping (1°/2°)
ALTER TABLE liquidacion_detalle ADD COLUMN IF NOT EXISTS inscripcion_id    UUID REFERENCES inscripciones(id);  -- caballo/SPC + carrera del premio (doc §7); NULL para incentivos por-reunión
ALTER TABLE liquidacion_detalle ADD COLUMN IF NOT EXISTS fecha_liberacion  DATE;             -- retención anti-doping
...
-- Beneficiario denormalizado (Decisión C1): para agrupar el recibo por persona
-- cruzando reuniones sin joinear al padre. beneficiario_id es polimórfico (sin FK).
...
-- reunion_id denormalizado (incentivos tienen carrera_id NULL): facilita resumen + recibo
ALTER TABLE liquidacion_detalle ADD COLUMN IF NOT EXISTS reunion_id        UUID REFERENCES reuniones(id);

-- Índices para la agregación del recibo (impagas por persona) y por recibo
...
-- ─────────────────────────────────────────────────────────────────────────
-- 6. RLS (patrón 2A club_id directo — fn_get_user_club_id / fn_is_super_admin)
-- ─────────────────────────────────────────────────────────────────────────
...
-- club_secuencias: se escribe vía fn_siguiente_recibo (SECURITY DEFINER bypasea RLS).
-- Acceso directo restringido al club / super_admin.
...
-- ─────────────────────────────────────────────────────────────────────────
-- 7. Auditoría (mismo patrón que trg_audit_liquidaciones → fn_auditoria_log)
-- ─────────────────────────────────────────────────────────────────────────
...
-- ============================================================================
-- NOTA "oficializar reunión" (Fase 2bis): NO requiere schema. Se resuelve con
-- un botón que setea resultados.estado='oficial' en bloque (columna existente).
