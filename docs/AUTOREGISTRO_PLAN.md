...
⚠️ **Requisito de diseño — inscripciones múltiples** (confirmado por Yesica el 04/08; GOTCHA #69, ISSUE-048): un caballo **se anota en varias categorías de la misma reunión** y recién el **lunes previo** la secretaría decide en cuál queda. El RPC de inscripción **no** puede validar unicidad por reunión — el schema tampoco lo hace (`UNIQUE (carrera_id, spc_id)`, por carrera). En prod ya pasa: R6 del 20/06 tiene 13 ejemplares en 2 turnos cada uno. Si hace falta, aviso informativo en la UI; nunca un rechazo.

**El portal anota, la secretaría resuelve** (Leo, 04/08). El entrenador **sí** puede anotar el mismo caballo en varias categorías desde el portal — es el proceso real. La **resolución del lunes** (elegir la categoría definitiva y dar de baja las otras inscripciones) queda **fuera del portal**: la hace la secretaría desde el back office. El Gate 4 **no** necesita UI de baja para esto; su alcance es anotar y mostrar lo ya anotado.
