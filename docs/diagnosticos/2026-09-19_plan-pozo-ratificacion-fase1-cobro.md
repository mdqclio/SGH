...
| Premisa a corregir | El pedido dice *"mismo criterio que el gate de jockeys: avisa, no bloquea"*. **El gate de jockeys BLOQUEA** (`resultados.html:1623-1643`: `confirm` → `return`, sin "seguir igual"; el comentario lo dice: *"Bloqueo duro y no advertencia"*). Para el pozo se construye lo pedido —aviso— pero es un criterio **distinto** al de montas, no el mismo. |
...
- **Corrección a la premisa del pedido:** el gate de montas (`:1623-1643`) hace `confirm(...) → return` sin opción de seguir: es bloqueo duro, y su comentario lo justifica ("el 20/09 la carga la hace alguien que usa SGH por primera vez, que a una advertencia le da Aceptar"). El aviso del pozo es deliberadamente **más blando**. Vale la pena decidir a conciencia que el mismo usuario que "le da Aceptar" va a poder oficializar con cobros faltantes — es lo pedido, y es razonable porque el faltante queda visible y cobrable después en `pozo.html`, cosa que con el jockey no pasaba (la línea se perdía en silencio).
