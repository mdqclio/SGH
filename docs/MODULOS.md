...
### portal.html — Modificar inscripción (🔧 16/09/2026, pedido de Yesi 14/09 — rama `feat/portal-modificar-inscripcion`, RPC pendiente de `apply_migration`)
**Botón Modificar** en Mis inscripciones, al lado de Retirar/Dar forfait, con el **mismo predicado** (`modoRetiro(i) !== null`: propia + `canal='portal'` + ventana de inscripción o de ratificación + estado admitido). Modal `#modal-modificar` con caballeriza / entrenador que presenta / jockey / suplente prellenados. **El SPC y el turno no se cambian** (retirar y anotar de nuevo). Toda la escritura va por `rpc_modificar_inscripcion` (`SECURITY DEFINER`, `migrations/rpc_modificar_inscripcion.sql`): mismos guards que `rpc_baja_inscripcion`, validaciones de padrón de `rpc_inscribir`, `FOR UPDATE`, un `ratificado` no queda sin jockey. **Tenencia = `inscripto_por` (quién la cargó), no `entrenador_id`** — cambiar el entrenador no la transfiere (GATE-1 = B, ISSUE-082); el `confirm()` lo dice así. **Caballeriza sin titular**: aviso inline al elegirla + `confirm()` al guardar + toast warning si el RPC devuelve `sin_propietario=true` (el trigger deja `propietario_id` NULL en silencio, GOTCHA #47; no se bloquea, lo regulariza la secretaría). Jockey repetido en el turno: mismo aviso que al anotar, excluyendo la propia fila. La secretaría sigue editando cualquier fila por `inscripciones.html` sin ventana. Probe `tests/probe_modificar_inscripcion_portal.mjs` (`PORTAL_HTML` acepta URL → corre contra el HTML servido).

### caballerizas.html — titular opcional + provisorio automático (🔧 16/09/2026, rama `feat/caballeriza-titular-opcional-provisorio`)
**Propietario opcional (todo-o-nada)** en el alta: los tres vacíos = "no lo conozco" → al guardar se llama a `rpc_caballeriza_provisorio` y la caballeriza queda con un **propietario provisorio** a su nombre (criterio Fede 15/08; misma forma que los 47 del `DO` de R8/R9: `tipo persona`, sin DNI, `notas 'provisorio alta DD/MM/YYYY'`). Si se carga alguno de los tres, se exigen los tres (el DNI es la llave del trigger). **Edición**: `openModal` toma un snapshot del bloque de responsables; si al guardar está igual, **no se valida ni se toca `caballeriza_responsables`** — antes el form hacía `delete` + `insert` siempre y con un provisorio (sin DNI) lo borraba o creaba otro propietario (ISSUE-080). Aviso en el modal cuando el titular es provisorio. Cards: badge `⚠ sin titular` / `◐ provisorio` y botón **Crear provisorio** (mismo RPC) en las que no tienen titular. Completar un provisorio con el DNI real desde la ficha sigue siendo ISSUE-080 (crea otro propietario). Probe `tests/probe_caballeriza_provisorio.mjs`.
